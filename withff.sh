...
# setup_novnc.sh
# Installs XFCE + VNC + noVNC + Firefox on a RunPod instance (Ubuntu/Debian based)
...
echo "=== [1/6] Updating packages, configuring Mozilla PPA, and installing XFCE + VNC + Firefox + noVNC ==="
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y software-properties-common

# Add Mozilla PPA to get true apt-based Firefox instead of snap
add-apt-repository -y ppa:mozillateam/ppa

# Set high pin priority to ensure Firefox is pulled from PPA, not snap/ubuntu repos
...
echo " Browser      : Mozilla Firefox (PPA)"
echo ""
echo " Next steps:"
